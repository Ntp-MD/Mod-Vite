...
export function useDemoWebsite() {
      ...
      Papa.parse(res.data as string, {
        ...
        complete: (results: Papa.ParseResult<Record<string, string>>) => {
          sheetRows.value = results.data
            .filter((row: Record<string, string>) => row.Name)
  ...
  // Filter range
  function filterRange(min: number, max: number): void {
    filterMin.value = min;
    filterMax.value = max;
  }
  ...
  const filteredData = computed(() => {
    return sheetRows.value
      .filter((item) => {
        ...
        const inRange = (filterMin.value === null && filterMax.value === null) || (filterMin.value !== null && filterMax.value !== null && idNum >= filterMin.value && idNum <= filterMax.value);
  ...
  // Status detection
  function statusDetect(value: string): string {
    if (!value) return "";
