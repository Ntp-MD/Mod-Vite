  ...
  <div class="wrapper-container">
    <div class="wrapper-scroll">
      <div class="results-display">
        ...
        <div
          class="show-stat"
          @click="showWaiting = showWaiting === '5gb' ? '' : '5gb'"
        >
          <span class="result-label">เพิ่มพื้นที่ 5GB</span>
          <span class="show-stat-count">{{ waitingSpace5GB }}</span>
          <div
            v-if="showWaiting === '5gb' && waitingList5GB.length"
            class="show-list"
          >
            <div v-for="row in waitingList5GB" :key="row.id">
              {{ row.selectName }}
            </div>
          </div>
        </div>
        ...
        <div
          class="show-stat"
          @click="showWaiting = showWaiting === 'sc' ? '' : 'sc'"
        >
          <span class="result-label">Search Console</span>
          <span class="show-stat-count">{{ waitingSearchConsole }}</span>
          <div
            v-if="showWaiting === 'sc' && waitingListSC.length"
            class="show-list"
          >
            <div v-for="row in waitingListSC" :key="row.id">
              {{ row.selectName }}
            </div>
          </div>
        </div>
        ...
        <div
          class="show-stat"
          @click="showWaiting = showWaiting === 'sw' ? '' : 'sw'"
        >
          <span class="result-label">Smart Widget</span>
          <span class="show-stat-count">{{ waitingSmartWidget }}</span>
          <div
            v-if="showWaiting === 'sw' && waitingListSW.length"
            class="show-list"
          >
            <div v-for="row in waitingListSW" :key="row.id">
              {{ row.selectName }}
            </div>
          </div>
        </div>
      ...
      <div class="pagination-container">
        <div class="pagination-info">
          Showing {{ displayFrom }}–{{ displayTo }} of {{ filteredCount }}
        </div>
    ...
    <div class="filter-sidebar">
      <div class="filter-section">
        ...
        <div class="filter-actions">
          ...
          <button
            :class="['btn-toggle', { active: showLast100 }]"
            @click="showLast100 = !showLast100"
          >
...
<script setup lang="ts">
  ...
  const waitingList5GB = computed(() =>
    (filteredRows.value || []).filter(
      (row) => row.selectfree5GB === 'เพิ่มพื้นที่ (5 GB)' && !row.Check5GB
    )
  )
  const waitingListSC = computed(() =>
    (filteredRows.value || []).filter(
      (row) =>
        ...
        (!row.onlineSearchConsole || row.onlineSearchConsole === 'Wait')
    )
  )
  const waitingListSW = computed(() =>
    (filteredRows.value || []).filter(
      (row) =>
        row.selectfreeSmartWidget === 'Smart Widget' &&
