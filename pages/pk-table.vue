<template>
  <div class="TableLayout">
          ...
          <div class="PlayerHand">
            <div
              ...
              :class="[
                ...
                { hideCard: i !== 0 && gamePhase !== 'showdown' }
              ]"
            >
              <div class="RankCard">{{ card.rank }}</div>
              <div class="SuitCard">{{ card.suit }}</div>
    ...
    <div class="ActionButton">
      <button @click="playerAction('check')" :disabled="!canCheck">
        Check
      </button>
      <button @click="playerAction('call', callAmount)" :disabled="!canCall">
        Call ${{ callAmount }}
      </button>
      <button @click="playerAction('raise', raiseInput)" :disabled="!canRaise">
        Raise ${{ raiseInput }}
      </button>
      <button @click="playerAction('all-in')" :disabled="!canAll">
        All-In
      </button>
      <button
        @click="playerAction('fold')"
        :disabled="gamePhase === 'idle' || currentPlayer !== 0"
      >
        Fold
      </button>
    </div>
    ...
    <div class="TableSetting">
      <button @click="startGame" :disabled="gamePhase !== 'idle'">
        Start Game
      </button>
      <button @click="startNewRound" :disabled="gamePhase !== 'showdown'">
        Next
      </button>
...
<script setup lang="ts">
  import {
    ...
    currentPlayer,
    playerFolded
  } from '~/features/poker/Table'
  import '~/features/poker/poker-table.css'

  const timelineLogRef = ref<HTMLElement | null>(null)
